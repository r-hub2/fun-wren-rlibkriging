...
double NuggetKriging::_logLikelihood(const arma::vec& _theta_alpha,
  ...
  if (grad_out != nullptr) {
    ...
    // Finalize gradients
    for (arma::uword k = 0; k < d; k++) {
      terme1.at(k) = term1_vec.at(k) / (_sigma2 + _nugget);
  ...
  if (optim == "none") {  // just keep given theta, no optimisation of ll (but estim beta still possible)
    ...
    if (n_cpu > 0 && multistart > 1) {
      ...
      if (Optim::log_level > Optim::log_none) {
        Rcpp::Rcout << "Threads per worker: " << threads_per_worker 
    ...
    if (Optim::log_level > Optim::log_none) {
      Rcpp::Rcout << "Preallocating " << multistart << " KModel structures (n=" 
    ...
    auto optimize_worker = [&](arma::uword start_idx) -> OptimizationResult {
      ...
      try {
        ...
        if (Optim::log_level > Optim::log_none) {
          Rcpp::Rcout << "BFGS (start " << (start_idx+1) << "/" << multistart << "):" << arma::endl;
          Rcpp::Rcout << "  objective: " << m_objective << arma::endl;
          Rcpp::Rcout << "  max iterations: " << optimizer.max_iter << arma::endl;
          Rcpp::Rcout << "  null gradient tolerance: " << optimizer.pgtol << arma::endl;
          Rcpp::Rcout << "  constant objective tolerance: " << optimizer.factr * 1E-13 << arma::endl;          Rcpp::Rcout << "  reparametrize: " << Optim::reparametrize << arma::endl;
          ...
          Rcpp::Rcout << "  start_point: " << theta0.row(start_idx % multistart) << "";
          Rcpp::Rcout << "               " << alpha0[start_idx % alpha0.n_elem] << arma::endl;
        }
        ...
        while (retry <= Optim::max_restart) {
          ...
          if (Optim::log_level > Optim::log_info) {
            Rcpp::Rcout << "  Start " << (start_idx + 1) << ", Retry " << (retry)
          ...
          // near a boundary, that's likely the true optimum
          const double boundary_tol = 1e-6;  // Relaxed from arma::datum::eps (~1e-16)

          bool at_theta_lb = (sol_to_lb_theta < boundary_tol);
          bool at_theta_ub = (sol_to_ub_theta < boundary_tol);
          bool at_alpha_lb = (sol_to_lb_alpha < boundary_tol);
          bool at_alpha_ub = (sol_to_ub_alpha < boundary_tol);

          // Only retry if BFGS failed abnormally, not if it legitimately converged to boundary
          // BFGS convergence means gradient is small (checked by pgtol), so boundary is optimal
          bool should_retry = false;
          if (retry < Optim::max_restart) {
            if (opt_result.task.rfind("ABNORMAL_TERMINATION_IN_LNSRCH", 0) == 0) {
              should_retry = true;  // Abnormal termination, always retry
            } else if (opt_result.num_iters <= 2) {
              should_retry = true;  // Too few iterations, likely bad start point
            } else if (opt_result.f_opt > best_f_opt) {
              should_retry = true;  // No improvement over previous attempts
            }
            // Removed boundary checks from retry logic - BFGS convergence is sufficient
          }

          if (should_retry) {

            if (Optim::log_level > Optim::log_none) {
              Rcpp::Rcout << "  Restarting BFGS (start " << (start_idx+1) << ", retry " << (retry+1)
        ...
        if (Optim::log_level > Optim::log_none) {
          Rcpp::Rcout << "Warning: start point " << (start_idx + 1) << " failed: " << e.what() << arma::endl;
        }
      }
    ...
    if (multistart == 1) {
      ...
      if (Optim::log_level > Optim::log_none) {
        Rcpp::Rcout << "Thread pool: " << pool_size << " workers (ncpu=" << n_cpu 
